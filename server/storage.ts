...
export class DatabaseStorage {
  ...
  async unhideEvent(id: number): Promise<boolean> {
    const result = await db
      .update(events)
      .set({ isHidden: false })
      .where(eq(events.id, id))
  ...
  async createResidencyBooking(insertResidencyBooking: InsertResidencyBooking): Promise<ResidencyBooking> {
    const [booking] = await db
      .insert(residencyBookings)
      .values(insertResidencyBooking)
      .returning();
    return booking;
  }

  async getResidencyBookings(): Promise<ResidencyBooking[]> {
    return await db
      .select()
  ...
  async updateEventDescription(eventId: number, description: string): Promise<void> {
    await db.update(events).set({ description }).where(eq(events.id, eventId));
  }
  ...
  async upsertLumaHost(host: InsertLumaHost): Promise<LumaHost> {
    const [result] = await db
      .insert(lumaHosts)
      .values({ ...host, syncedAt: new Date() })
      .onConflictDoUpdate({
        target: lumaHosts.lumaHostId,
        set: {
          name: host.name,
          email: host.email,
          avatarUrl: host.avatarUrl,
          eventExternalIds: host.eventExternalIds,
          syncedAt: new Date(),
        },
      })
      .returning();
  ...
  async getAllLumaHosts(): Promise<LumaHost[]> {
    return db.select().from(lumaHosts).orderBy(lumaHosts.name);
  }
}
