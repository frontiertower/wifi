...
Example: {"results": [{"id": 1, "segments": ["AI", "Founders"]}, {"id": 2, "segments": ["Music", "Parties"]}]}`
  ...
  // Sync Luma hosts from event detail API
  app.post("/api/admin/hosts/sync", verifyAdminSession, async (req, res) => {
    try {
      const apiKey = process.env.LUMA_API_KEY;
      if (!apiKey) return res.status(400).json({ success: false, message: "LUMA_API_KEY not configured" });

      const { lumaGet } = await import('./luma-client');
      const allEvents = await storage.getAllEventsWithSegments();
      const eventsWithId = allEvents.filter(e => e.externalId && !e.isHidden);

      // Map: lumaHostId -> accumulated host data
      const hostMap = new Map<string, {
        name: string | null;
        email: string | null;
        avatarUrl: string | null;
        eventExternalIds: Set<string>;
      }>();
      ...
      for (const event of eventsWithId) {
        try {
          const data = await lumaGet<{
            event: unknown;
            hosts?: Array<{ api_id: string; name?: string; email?: string; avatar_url?: string }>;
          }>(
            `https://api.lu.ma/public/v1/event/get?event_api_id=${event.externalId}`,
            apiKey,
          );

          for (const host of (data.hosts ?? [])) {
            if (!host.api_id) continue;
            if (!hostMap.has(host.api_id)) {
              hostMap.set(host.api_id, {
                name: host.name ?? null,
                email: host.email ?? null,
                avatarUrl: host.avatar_url ?? null,
                eventExternalIds: new Set(),
              });
            }
            const entry = hostMap.get(host.api_id)!;
            // Prefer the most recent name/email if available
            if (host.name) entry.name = host.name;
            if (host.email) entry.email = host.email;
            if (host.avatar_url) entry.avatarUrl = host.avatar_url;
            if (event.externalId) entry.eventExternalIds.add(event.externalId);
          }
          synced++;
        } catch {
          errors++;
      ...
      // Upsert all collected hosts
      for (const [lumaHostId, data] of hostMap.entries()) {
        await storage.upsertLumaHost({
          lumaHostId,
          name: data.name,
          email: data.email,
          avatarUrl: data.avatarUrl,
          eventExternalIds: Array.from(data.eventExternalIds),
        });
      }

      res.json({
        success: true,
        message: `Scanned ${synced} events, found ${hostMap.size} unique host${hostMap.size !== 1 ? 's' : ''}${errors ? ` (${errors} events skipped)` : ''}`,
        hostsFound: hostMap.size,
        eventsScanned: synced,
      });
    } catch (error) {
      console.error("Error syncing Luma hosts:", error);
      res.status(500).json({ success: false, message: error instanceof Error ? error.message : "Failed to sync hosts" });
    }
  });

  // Get all hosts enriched with events and categories
  app.get("/api/admin/hosts", verifyAdminSession, async (req, res) => {
    try {
      const hosts = await storage.getAllLumaHosts();
      const allEvents = await storage.getAllEventsWithSegments();

      // Build lookup: externalId -> event
      const eventByExtId = new Map(allEvents.filter(e => e.externalId).map(e => [e.externalId!, e]));

      const enriched = hosts.map(host => {
        const hostedEvents = (host.eventExternalIds ?? [])
          .map(extId => eventByExtId.get(extId))
        ...
        return {
          ...host,
          hostedEvents: hostedEvents.map(ev => ({
            id: ev.id,
            name: ev.name,
          ...
          segments,
          eventCount: hostedEvents.length,
        };
      });

      // Collect all unique segment names for filters
      const allSegmentNames = Array.from(new Set(enriched.flatMap(h => h.segments.map(s => s.name)))).sort();

      res.json({ success: true, hosts: enriched, allSegmentNames });
    } catch (error) {
      console.error("Error fetching Luma hosts:", error);
