...
*   **Unified Guest Form**: Combines guest registration types into a single progressive disclosure form with conditional fields and multi-step password verification. Flow progression: Form → Password Screen → Congratulations Page → Success Modal. Supports dual default passwords ("makesomething" and "frontiertower995") when no custom admin password is set. Error recovery preserves form data and allows password retry without data loss.
