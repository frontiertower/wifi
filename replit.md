...
*   **Photo Gallery**: A `/gallery` route displays photos from the Frontier Tower iCloud shared album (token: B2NGq6kMgGcF56U). Uses the `icloud-shared-album` npm package to scrape public album data server-side. Features a responsive grid layout with thumbnails, lightbox viewer for full-size images with keyboard navigation (arrow keys, escape), and 5-minute server-side caching to reduce API calls. Accessible from homepage via dedicated button with rose/pink styling.
...
*   **Connected Devices**: Admin "Devices" tab fetches all client devices currently connected to the UniFi network via `GET /api/admin/devices`. Works with both the Modern API (Bearer token) and Legacy API (cookie-based auth). Returns normalized device records: MAC address, IP, hostname, connection type (Wireless/Wired), signal strength (dBm with color-coded quality), uptime, SSID/network name, and AP MAC. Supports paginated fetching for large deployments (up to 4000 devices). Includes a search bar (hostname, MAC, IP, SSID) and Wireless/Wired filter buttons. If the controller is not configured, a clear "not configured" message is shown with setup instructions. A manual Refresh button re-fetches live data on demand.
*   **Luma Guest Sync**: Admin Guests tab has a "Sync Guests" button that calls `GET https://api.lu.ma/public/v1/event/get-guests` for each Luma event, storing results in the `luma_guests` table. Guests are displayed in a filterable table showing name, email, event, approval status, registration date, and check-in date. Supports filtering by event name and free-text search by name/email. Supports pagination via `next_cursor`.
*   **Event Segments**: Admin Segments tab uses AI (GPT-4o) to categorize all events by topic. The AI is given a large seed list (~70 example categories) but is free to invent new ones for events that don't fit. Results stored in `segments text[]` column on events table. Segments tab displays color-coded summary tiles (with hash-based color fallback for AI-invented categories) and expandable detail sections per category. Each event row shows all its tags as removable chips with an inline "add tag" input (autocompleted from all known categories). Tag changes call `PATCH /api/admin/events/:id/segments` to persist immediately.