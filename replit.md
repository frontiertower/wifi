...
*   **Luma Host Sync**: Admin Hosts tab has a "Sync Hosts" button that calls `GET https://api.lu.ma/public/v1/event/get` for each Luma event, extracting structured host profiles (Luma user ID, name, email, avatar) and storing them in the `luma_hosts` table. Each host record tracks which events they hosted via `eventExternalIds text[]`. The Hosts tab shows a filterable table with name, avatar, category pills (ranked by frequency), and event count. Clicking a host opens a profile sheet showing their specialties and full event history. Category filters come from segments of events they hosted.
